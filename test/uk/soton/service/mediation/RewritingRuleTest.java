...
import com.hp.hpl.jena.rdf.model.AnonId; import com.hp.hpl.jena.sparql.algebra.Algebra;
...
public class RewritingRuleTest {
	...
	public void setUp() throws Exception {
		...
		Triple lhs = new Triple(pid1, Node.createURI("http://foaf.org/name"),
				name);
		@SuppressWarnings("serial")
		final FunctionalDependency fd = new FunctionalDependency(pid2,
				FunctionWrapper.$(new SameAs()), new ArrayList<Node>() {
					{
						add(pid1);
//						add(Node.createLiteral("http://dbpedia.org/*")); // Line changed by JB to:
						add(Node.createLiteral("http://dbpedia.org/\\S*")); // End change: correct regexp
					}
				}, RDFVocabulary.SAMEAS);
		@SuppressWarnings("serial")
		ArrayList<Triple> rhs = new ArrayList<Triple>() {
			{
				add(new Triple(pid2,
						Node.createURI("http://dbpedia.org/onto/name"), name));
			}
		};
		@SuppressWarnings("serial")
		RewritingRule rr = new RewritingRule().setLHS(lhs).setRHS(rhs)
				.setFD(new ArrayList<FunctionalDependency>() {
					{
						add(fd);
					}
				});
